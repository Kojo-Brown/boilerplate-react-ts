    ...
    >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="flex flex-col gap-2 rounded-lg border border-[var(--color-border)] p-4">
            <Skeleton variant="text" className="h-4 w-24" aria-label={`Loading stat label ${i + 1}`} />
